...
import { fetchFromCategory, fetchFromFood } from "../actions/creator";
...
export default function EditFood() {
  ...
  useEffect(() => {
    return async () => {
      const dataFood = await fetch(`http://localhost:3000/item/${id}`, {
        method: "GET",
        headers: {
      ...
      const dataFoodEdit = await dataFood.json();
      // console.log(dataFoodEdit, 'dataFoodEdit ini dia');
      setDataEdit(dataFoodEdit);
      // console.log(dataEdit, 'dataEdit ini dia');
              ...
              <form onSubmit={handleSubmit(addFood)} className="space-y-1">
                <div>
                  ...
                  <div className="mt-1">
                    <input
                      id=""
                      // defaultValue={dataFoodEdit.name}
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <select defaultValue={null} className="select select-bordered w-full ">
                      <option disabled hidden value={null}>Select Category</option>
                      {category.map((dataCategory) => (
                        <option {...register("categoryId")} key={dataCategory.id} value={dataCategory.id}>
                          {dataCategory.name}
                        </option>
                      ))}
                    </select>
